...
data = corpus.SentenceProvider(shuffle=True, save_path='./files/pickle')
...
net.add_layer(model.layers.LSTM(n_in=300, n_out=200))
net.add_layer(model.layers.LSTM(n_in=200, n_out=300))
net.add_layer(model.layers.MaxPooling(pool_size=(30, 1)))
net.add_layer(model.layers.Flatten(ndim=2))
# net.add_layer(model.layers.Mean(1))
net.add_layer(model.layers.Softmax(n_in=300, n_out=len(data.index_to_tag)))
